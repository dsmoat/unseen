...
export class DeterministicService implements IntelligenceService {
  async extractSignals(_messages: Message[]) {
    return signals;
  }
  async extractClaims(_messages: Message[]) {
    return claims;
  }
  async resolveEntities(_signals: unknown[], _participants: unknown[]) {
    return [];
  }
  async generateOpportunity(_patterns: unknown, _capabilities: unknown[]) {
    return opportunities;
  }
  async generateValidationQuestions(_opportunity: Opportunity) {
    return [
  ...
  async generateExperiment(o: Opportunity, _answers: unknown[]) {
    return {
      opportunityId: o.id,
